...
libraries <- c("stringr", "dplyr", "shiny")
...
ui <- fluidPage(
    # Application title
    titlePanel(h1("RASopaties: Classificació de variants missense", align = "center")),
    tags$br(),
    # Sidebar with a slider input 
    sidebarLayout(
        sidebarPanel(
            selectizeInput(inputId = "gen", label = "Seleccioni un gen", choices = list('BRAF', 'CBL', 'HRAS', 'KRAS', 'LZTR1', 'NF1', 'NRAS', 'MAP2K2', 'MAP2K1', 'PTPN11', 'RAF1', 'RIT1',
                                                                                        'SHOC2', 'SOS1', 'SOS2', 'SPRED1', 'RASA1'), multiple = F),
            textInput(inputId = "variant_cDNA", label = "Introdueixi la variant en format cDNA", placeholder = 'ex: c.171T>A'),
            textInput(inputId = "variant_prot", label = "Introdueixi la variant en format proteïna", placeholder = 'ex: (p.Phe57Leu)'),
            numericInput(inputId = "de_novo", label = "Quants casos de novo s'han trobat?", value = 0, min = 0),
            numericInput(inputId = "confirmats", label = "Quants casos tenen confirmació parental?", value = 0, min = 0),
            selectInput(inputId = "estudis_funcionals", label = "Hi ha estudis funcionals demostrant la seva patogenicitat?", choices = list("No" = 0, "Sí" = 1), selected = 0),
            helpText("Prevalença de la variant en individus afectats:"),
            numericInput(inputId = "casos_independents", label = "Quants casos independents s'han trobat?", value = 0, min = 0),
            numericInput(inputId = "individus_control", label = "Quants casos en individus control?", value = 0, min = 0),
            helpText("Hi ha cosegregació de la variant?"),
            numericInput(inputId = "meiosis", label = "Indiqui el número de meiosis", value = 0, min = 0),
            selectInput(inputId = "computational_evidence_FM", label = "Hi ha evidències computacionals de que la variant Sí compromet la funció de la proteïna?", 
                        choices = list("No" = 0, "Sí" = 1), selected = 0),
            selectInput(inputId = "computational_evidence_FI", label = "Hi ha evidències computacionals de que la variant NO compromet la funció de la proteïna?", 
                        choices = list("No" = 0, "Sí" = 1), selected = 0),
            selectInput(inputId = "computational_evidence_ALT", label = "Hi ha evidències computacionals demostrant la seva patogenicitat?", choices = list("No" = 0, "Sí" = 1), 
                        selected = 0),
            helpText("La variant es troba en trans amb una variant patogènica d'un gen amb penetrància completa dominant o en cis amb una variant amb qualsevol tipus d'herència?"),
            selectInput(inputId = "trans_cis", label = "cis / trans", choices = list("No" = 0, "Sí" = 1), selected = 0),
            actionButton("go", "Cerca")), 
        mainPanel(tags$h4(tags$style('h4 {color:grey;}')),
            fluidRow(column(9, tags$h4("Resum dels criteris utilitzats per la classificació"), offset = 2), column(9, tableOutput("Criteris"), offset = 2)),
            tags$hr(),
            fluidRow(column(9, tags$h4("Classificació segons les guies ACMG-AMP"), offset = 2), column(9, tableOutput("Classificacio"), offset = 2)))))
...
server <- function(input, output) {
    ...
    Criteris_ACMG_AMP <- function(gen=as.character(input$gen),variant_cDNA=as.character(input$variant_cDNA),variant_prot=as.character(input$variant_prot),
                                  de_novo=input$de_novo,confirmats=input$confirmats,estudis_funcionals=as.numeric(as.character(input$estudis_funcionals)),
                                  casos_independents=input$casos_independents,individus_control=input$individus_control,meiosis=input$meiosis,
                                  computational_evidence_FM=as.numeric(as.character(input$computational_evidence_FM)),trans_cis=as.numeric(as.character(input$trans_cis)),
                                  computational_evidence_FI=as.numeric(as.character(input$computational_evidence_FI)),computational_evidence_ALT=as.numeric(as.character(input$computational_evidence_ALT))){
        ...
        # PM2 / BA1 / BS1 Les dades de freqüència al·lèlica les consulta de gnomad
    ...
    Classificacio_ACMG_AMP <- function(gen=as.character(input$gen),variant_cDNA=as.character(input$variant_cDNA),variant_prot=as.character(input$variant_prot),
                                       de_novo=input$de_novo,confirmats=input$confirmats,estudis_funcionals=as.numeric(as.character(input$estudis_funcionals)),
                                       casos_independents=input$casos_independents,individus_control=input$individus_control,meiosis=input$meiosis,
                                       computational_evidence_FM=as.numeric(as.character(input$computational_evidence_FM)),trans_cis=as.numeric(as.character(input$trans_cis)),
                                       computational_evidence_FI=as.numeric(as.character(input$computational_evidence_FI)),computational_evidence_ALT=as.numeric(as.character(input$computational_evidence_ALT))){
        ...
        criteris <- Criteris_ACMG_AMP(gen=as.character(input$gen),variant_cDNA=as.character(input$variant_cDNA),variant_prot=as.character(input$variant_prot),
                                      de_novo=input$de_novo,confirmats=input$confirmats,estudis_funcionals=as.numeric(as.character(input$estudis_funcionals)),
                                      casos_independents=input$casos_independents,individus_control=input$individus_control,meiosis=input$meiosis,
                                      computational_evidence_FM=as.numeric(as.character(input$computational_evidence_FM)),trans_cis=as.numeric(as.character(input$trans_cis)),
                                      computational_evidence_FI=as.numeric(as.character(input$computational_evidence_FI)),computational_evidence_ALT=as.numeric(as.character(input$computational_evidence_ALT)))
    ...
    # Criteris
    Criteris_reactive <-eventReactive(c(input$go == 0,input$gen, input$variant_cDNA, input$variant_prot, input$de_novo, input$confirmats,
                                        input$estudis_funcionals,input$casos_independents, input$individus_control,input$meiosis,
                                        input$computational_evidence_FM,input$trans_cis,input$computational_evidence_FI,input$computational_evidence_ALT),
                                        if (input$gen == 0 | input$variant_cDNA == 0 | input$variant_prot == 0 |  input$go == 0){
                                            return(NULL)
                                        }
                                        else 
                                        {Criteris_ACMG_AMP(gen=as.character(input$gen),variant_cDNA=as.character(input$variant_cDNA),variant_prot=as.character(input$variant_prot),
                                                                       de_novo=input$de_novo,confirmats=input$confirmats,estudis_funcionals=as.numeric(as.character(input$estudis_funcionals)),
                                                                       casos_independents=input$casos_independents,individus_control=input$individus_control,meiosis=input$meiosis,
                                                                       computational_evidence_FM=as.numeric(as.character(input$computational_evidence_FM)),trans_cis=as.numeric(as.character(input$trans_cis)),
                                                                       computational_evidence_FI=as.numeric(as.character(input$computational_evidence_FI)),computational_evidence_ALT=as.numeric(as.character(input$computational_evidence_ALT)))
    ...
    criteri <- renderTable(expr = Criteris_reactive(),rownames = FALSE, bordered = FALSE)
    output$Criteris <- criteri
    
    # Classificació
    Classificacio_reactive <-eventReactive(c(input$go == 0,input$gen, input$variant_cDNA, input$variant_prot, input$de_novo, input$confirmats,
                                        input$estudis_funcionals,input$casos_independents, input$individus_control,input$meiosis,
                                        input$computational_evidence_FM,input$trans_cis,input$computational_evidence_FI,input$computational_evidence_ALT),
                                      if (input$gen == 0 | input$variant_cDNA == 0 | input$variant_prot == 0 |  input$go == 0){
                                          return(NULL)
                                      ...
                                      {Classificacio_ACMG_AMP(gen=as.character(input$gen),variant_cDNA=as.character(input$variant_cDNA),variant_prot=as.character(input$variant_prot),
                                                         de_novo=input$de_novo,confirmats=input$confirmats,estudis_funcionals=as.numeric(as.character(input$estudis_funcionals)),
                                                         casos_independents=input$casos_independents,individus_control=input$individus_control,meiosis=input$meiosis,
                                                         computational_evidence_FM=as.numeric(as.character(input$computational_evidence_FM)),trans_cis=as.numeric(as.character(input$trans_cis)),
                                                         computational_evidence_FI=as.numeric(as.character(input$computational_evidence_FI)),computational_evidence_ALT=as.numeric(as.character(input$computational_evidence_ALT)))
    ...
    class <- renderTable(expr = Classificacio_reactive(),rownames = FALSE, bordered = FALSE)
    output$Classificacio <- class
